    ...
    def parse_page_index(self, number):
        ...
        for item in self._get_page_index(from_station_no, to_station_no):
            ...
            if result_list[0] != '' and result_list[0] != 'null':
                ...
                index += 1
        print('如果您购买的是学生票，请选择乘车区间内的二等座及其以下的车票，不在乘车区间或者一等座及其以上只能购买成人票！！！')
        while True:
            if number == 0:
                # 查询价格
                while True:
                ...
                while True:
                    ...
                    if category in [str(i) for i in range(1, 7)]:
                        if category in ['1', '2']:
                            self.is_adult = True
                        else:
                            while True:
                                flag = input('请再次确认，您购买的是学生票还是成人票（1代表成人，0代表学生）')
                                if flag == '1' or '0':
                                    break
                                else:
                                    print('输入不合法请重新输入')
        ...
        # secret_str
        print(all_train_own_ticket[order][0])
        secret_str = all_train_own_ticket[order][0]
        # train_no
        print(all_train_own_ticket[order][2])
        train_no = all_train_own_ticket[order][2]
        # stationTrainCode车次
        print(all_train_own_ticket[order][3])
        train_code = all_train_own_ticket[order][3]
        # fromStationTelecode出发站点
        print(all_train_own_ticket[order][6])
        from_station_code = all_train_own_ticket[order][6]
        # toStationTelecode到达站点
        print(all_train_own_ticket[order][7])
        to_station_code = all_train_own_ticket[order][7]
        # train_location
        print(all_train_own_ticket[order][15])
        train_location = all_train_own_ticket[order][15]
        print(category)
        print(remainder)
        print(type(remainder))
        # for i, item in enumerate(all_train_own_ticket[order]):
        #     print(i, item)
        return secret_str, train_no, train_code, from_station_code, to_station_code, \
            train_location, category, remainder, self.is_adult
